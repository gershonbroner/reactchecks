...
const HandleTerm = useCallback(() =>{
    ...
    return (
        ...
        flexDirection:"row",
         ...
         >
       <button onClick={()=>{setIsInputOpen(!isInputOpen)}} style={{ border: "none"}}><TbPointFilled /></button> 
       <div style={{background:`${color}`,borderRadius:"10px ",marginBottom:"5px" }} onClick={()=>deleteOrSetTerm(term)}>{term}</div>
       {obj&&<button style={{ border: "none", color:"green"}} onClick={()=>{setChooseColor(!choosecolor)}}><AiOutlineBgColors/></button>}
       {obj&&<button style={{ border: "none"}} onClick={()=>{setIsOpen(!isOpen)}}><FaLongArrowAltRight /></button> }
        </div>
        {choosecolor?<div style={{zIndex:"auto",position: 'absolute'}}><PickerColor sendColor={correctColor}/></div>:""}
        ...
        {
            isOpen && obj && Object.keys(obj).map((key:string)=>{
            return <Term key={key} term={key} obj={obj[key]} addTerm={addTerm} deleteOrSetTerm={deleteOrSetTerm} />
            })
        }
        </div>
        {isInputOpen&&
        <div>
        <input type="text"  onChange={(e)=>setNewTerm(e.target.value)} style={{ border: "none",borderBottom: "2px solid"}}></input>
        <button
         onClick={HandleTerm}
        style={{ border: "none"}}
        aria-label="Submit new sub-term">
        <TbPointFilled /></button> 
        </div> 
        }
        </div>
    )
 ...
 return false
    ...
    const addTerm =useCallback((idTarget: string, term: string) => {
        setObj((prevObj) => {
            ...
            const add = (obj: StructureMap, idTarget: string, newTerm: string): boolean => {
    ...
    return (
        <>
            <Term term={mainTerm} obj={baseObj[mainTerm]} addTerm={addTerm} deleteOrSetTerm={deleteOrSetTerm} />
        </>
    );
};
